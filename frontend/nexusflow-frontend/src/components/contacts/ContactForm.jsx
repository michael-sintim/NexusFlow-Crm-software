...
const ContactForm = ({ initialData }) => {
  ...
  const onSubmit = async (data) => {
    try {
      if (initialData) {
        await callApi(() => contactsAPI.update(initialData.id, data))
      } else {
        await callApi(() => contactsAPI.create(data))
      }
      // Navigate back to contacts page after successful creation/update
      navigate('/contacts')
    } catch (err) {
        ...
        <div className="mb-8">
          ...
          <div className="text-center">
            ...
            <p className="text-gray-600 dark:text-gray-400 mt-2">
              {initialData ? 'Update contact information' : 'Add a new contact to your network'}
            </p>
          </div>
        </div>
